...
results_store = {}
active_sessions = set() # 経路情報は持たず、セッションIDの存在確認のみ
...
def on_register_session(name, param, app_param):
    try:
        ...
        print(f"[Server] セッションを登録 (経路情報は受け取っていません): ID={session_id}", flush=True)
